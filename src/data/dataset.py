...
class ADNIDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        # sex_name = self.dataframe.sex.values[index]  # expects the dataframe in format (filename, label)
        # sex_idx = self.sex_to_idx[sex_name]
        # age = self.dataframe.age.values[index]
        # cdr_global = self.dataframe.cdr_global.values[index]
        # MMSE = self.dataframe.MMSE.values[index]
        ...
        return transformed_mri, class_idx#, sex_idx, age, cdr_global, MMSE  # return data, label (X, y)
