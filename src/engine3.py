...
def train_step(model: torch.nn.Module,
    ...
    # for (X, y, sex, age,_,_) in tqdm(dataloader, total=len(dataloader)):
    for (X, y) in tqdm(dataloader, total=len(dataloader)):
        ...
        # 1. Forward pass
        y_pred,t, m = model(X, training=True)
        # p1 = F.normalize(ad, dim=-1)
        # y_pred,t, m = model(X)
        # y_pred= model(X)


        # p2 = F.normalize(cn, dim=-1)
        #
        # # 损失计算
        # # 1. 推远两个原型（负的余弦相似度）
        # push_loss = -F.cosine_similarity(p1, p2).mean()
    
        loss1 = loss_fn(y_pred, y)
        # loss2 = loss_fn(t, y)
        # loss3 = loss_fn(m, y)

        # loss2 = loss_fn(t_pred, y)
